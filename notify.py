...
credentials = ServiceAccount(username='ckreuzberger@anx.local', password=password)

# Set up a target account and do an autodiscover lookup to find the target EWS endpoint:
account = Account(primary_smtp_address='ckreuzberger@anexia-it.com', credentials=credentials,
                  autodiscover=True, access_type=DELEGATE)

config = Configuration(server='exchange.anexia.at', credentials=credentials, auth_type=NTLM)
